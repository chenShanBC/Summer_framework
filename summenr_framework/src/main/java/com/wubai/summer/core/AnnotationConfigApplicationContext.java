...
public class AnnotationConfigApplicationContext {
        ...
        for (String className : classNames) { //逐一处理每一个全限定名
            try {
                ...
                if (!hasComponentAnnotation(clazz)) {
                    System.out.println("❌ 跳过无@Component的类：" + className);
                    continue;
                }
    ...
    private void registerBeanMethods(String factoryBeanName, Class<?> configClass) {
        ...
        for (Method method : configClass.getMethods()) {
            // 新增：打印所有遍历到的方法名（关键排查！）
            System.out.println("🔍 遍历到配置类方法：" + method.getName());
            ...
            beanDefinitionMap.put(beanName, beanDef);
            // 新增日志：确认注册成功
            System.out.println("✅ 成功注册@Bean：" + beanName + "，类型：" + method.getReturnType().getName());
        }
    }
